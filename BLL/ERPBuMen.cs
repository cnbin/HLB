...
namespace ZWL.BLL
{
    ...
    public class ERPBuMen
    {
        ...
        public void Update()
        {
            ...
            //DbHelperSQL.GetSHSL("select bumenname from ERPBuMen where id=" + ID + "");
            //System.Web.HttpContext.Current.Response.Write("update ERPUser set Department='" + parameters[1].Value + "' where Department='" + DbHelperSQL.GetSHSL("select BuMenName from ERPBuMen where id=" + ID + "") + "'");
            DbHelperSQL.ExecuteSql("update ERPUser set Department='" + parameters[1].Value + "' where Department='" + DbHelperSQL.GetSHSL("select BuMenName from ERPBuMen where id=" + ID + "") + "'");
            DbHelperSQL.ExecuteSql(strSql.ToString(), parameters);
        }
        ...
        ///
        /// </summary>
        public int GetBuMenID(string BuMenName)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,BuMenName,ChargeMan,TelStr,ChuanZhen,BackInfo,DirID ");
            strSql.Append(" FROM ERPBuMen ");
            strSql.Append(" where BuMenName='" + BuMenName + "' ");
            SqlParameter[] parameters = {
					new SqlParameter("@BuMenName", SqlDbType.VarChar) };
            parameters[0].Value = BuMenName;
